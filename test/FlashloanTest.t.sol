...
contract FlashloanTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory uniswapInitData = abi.encodeWithSelector(UniswapFlashSwap.initialize.selector, owner, 50, 10);
    ...
    function testFuzz_Initialization(address _owner, uint256 _feeBps, uint256 _minProfitBps) public {
    ...
    function testFuzz_UniswapInitialization(address _owner, uint256 _feeBps, uint256 _minProfitBps) public {
        // Bound values to valid ranges
        vm.assume(_owner != address(0));
        _feeBps = bound(_feeBps, 0, 1000);
        _minProfitBps = bound(_minProfitBps, 0, 1000);
        ...
        bytes memory uniswapInitData =
            abi.encodeWithSelector(UniswapFlashSwap.initialize.selector, _owner, _feeBps, _minProfitBps);
        ERC1967Proxy uniswapProxy = new ERC1967Proxy(address(uniswapImpl), uniswapInitData);
